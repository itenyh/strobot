...
class RobotAction {
    ...
    addListener() {
        ...
        this.pomelo.on('io-error', function (data) {
            logger.error('【%s】游戏机器人 => socket error , 原因: %s %s', this.getId(), data.code, data.reason)
            this.stop()
        }.bind(this))
    }
    ...
    play() {
        ...
        // return Q.async(function* () {
        //
        //     while (!this.memory.tempTerminatePaly && !this.rules.timeout(this.memory.entertime, this.memory.lifelong)) {
        //
        //         const pay = this.caculatePay(this.memory.gold)
        //         if (pay) {
        //
